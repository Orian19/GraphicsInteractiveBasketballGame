...
const leftBasketLight = createBasketLight(-15, 12, 3, -15, 3, 0);

// right basket lighting
const rightBasketLight = createBasketLight(15, 12, 3, 15, 3, 0);
...
function createBasketballCourt() {
  ...
  const lineHeight = 0.11; // court top surface (0.1) + half line thickness (0.01)
...
function createCourtBoundary(material, height) {
  ...
  */
  ...
  createCourtLine(material, 30, lineWidth, lineThickness, 0, height, 7.5);
  createCourtLine(material, 30, lineWidth, lineThickness, 0, height, -7.5);
  
  // end lines (short sides)
  createCourtLine(material, lineWidth, 15, lineThickness, 15, height, 0);
  createCourtLine(material, lineWidth, 15, lineThickness, -15, height, 0);
}
...
function createCenterLine(material, height) {
  ...
  */
  createCourtLine(material, 0.1, 15, 0.02, 0, height, 0);
}
  ...
  */
  ...
  function createThreePointSide(basketX) {
    ...
    */
    ...
    createCourtLine(material, 0.1, straightLineLength, 0.02, basketX, height, straightLineLength / 2);
    createCourtLine(material, 0.1, straightLineLength, 0.02, basketX, height, -straightLineLength / 2);
  }
...
function createThreePointArc(material, height, basketX, radius) {
  ...
  */
  ...
  const connectionGeometry = new THREE.BoxGeometry(connectionLength, 0.02, 0.1);
  
  const arcEndY = radius;
  
  if (arcEndY < 7.5) { // half the court width (15/2)
    // top connection line
...
function createSymmetricCourtElements(createElementFunc, ...args) {
  ...
  */
  ...
  createElementFunc(15, ...args);  // right side
  createElementFunc(-15, ...args); // left side
}
  ...
  */
  ...
  function createFreeThrowLine(basketX) {
    ...
    */
    ...
    createCourtLine(material, 0.1, keyWidth, 0.02, x, height, 0);
  }
...
function createKeyAreas(material, height) {
  ...
  */
  ...
  // right key area
  createKeyArea(material, height, 15, keyWidth, keyLength, lineWidth);
...
function createBasketballHoops() {
  ...
  */
  ...
  createBasketballHoop(15 - basketOffsetFromEndline, 0, -1); // right side hoop
  createBasketballHoop(-15 + basketOffsetFromEndline, 0, 1); // left side hoop
}
...
function createSupportStructure(x, z, rimHeight, backboardHeight, backboardWidth, backboardThickness, direction, poleRadius, supportOffset) {
  ...
  */
  ...
  ctx.fillStyle = '#ffffff';
  ctx.beginPath();
  ctx.arc(canvas.width/2, 25, 15, 0, degrees_to_radians(360));
  ctx.fill();
  ...
  const courtEdgeX = direction > 0 ? -15 : 15; // the court edge X position
...
function createBasketball() {
  ...
  */
  ...
  basketballGroup.position.set(0, basketballRadius + 0.1, 0);
...
instructionsContainer.innerHTML = `
  ...
  <h4>Ball Controls (Future)</h4>
  <p><span class="key-command">W</span>/<span class="key-command">S</span> Move ball up/down</p>
...
function handleKeyDown(e) {
  ...
  */
  ...
  // W/S/A/D keys
    ...
    feedbackMessage = `Key pressed: ${e.key.toUpperCase()} (move ball up - will be implemented in future)`;
  }
  else if (e.key === "s" || e.key === "S") {
    feedbackMessage = `Key pressed: ${e.key.toUpperCase()} (move ball down - will be implemented in future)`;
  }
  else if (e.key === "a" || e.key === "A") {
    feedbackMessage = `Key pressed: ${e.key.toUpperCase()} (move ball left - will be implemented in future)`;
  }
  else if (e.key === "d" || e.key === "D") {
    feedbackMessage = `Key pressed: ${e.key.toUpperCase()} (move ball right - will be implemented in future)`;
  }
  // arrow keys
  else if (e.key === "ArrowUp") {
    feedbackMessage = `Key pressed: ↑ (move ball forward - will be implemented in future)`;
  }
  else if (e.key === "ArrowDown") {
    feedbackMessage = `Key pressed: ↓ (move ball backward - will be implemented in future)`;
  }
  else if (e.key === "ArrowLeft") {
    feedbackMessage = `Key pressed: ← (move ball left - will be implemented in future)`;
  }
  else if (e.key === "ArrowRight") {
    feedbackMessage = `Key pressed: → (move ball right - will be implemented in future)`;
  }
  // space and R keys
  else if (e.key === " ") {
    ...
    feedbackMessage = `Key pressed: ${e.key.toUpperCase()} (reset ball position - will be implemented in future)`;
  }
